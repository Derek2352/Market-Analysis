"""Quora scraper — Playwright-based answer extraction.
...
Region-aware: the *region* parameter controls what search
terms are appended to the topic query (e.g. "Hong Kong", "United States")
and what region/language metadata appears on each RawPost.
"""
...
# Region → human-readable search term appended to the topic query.
...
# Region → language code for RawPost metadata.
...
class QuoraScraper:
    """Quora scraper — search for region-tagged questions, parse answer pages."""
    ...
    def __enter__(self) -> "QuoraScraper":
        return self
    ...
    def search(
        ...
        """Search Quora for region-relevant questions, parse each result."""
        query = f"{topic} {self._search_term}"
        ...
        try:
            ...
            _log.warning("quora.search_failed", topic=topic, error=str(e))
            return

        question_urls = parse_search_results(html)[: self._max_questions]
        _log.info(
            "quora.search", topic=topic, candidates=len(question_urls),
        )
        ...
        for qurl in question_urls:
            ...
            try:
                ...
                _log.warning(
                    "quora.question_failed", url=qurl, error=str(e),
                )
                continue
    ...
    def fetch_question(self, question_url: str) -> RawPost:
        ...
        with self._playwright.get_page(question_url) as page:
            ...
            html = page.content()
        return parse_question_page(
            html, question_url=question_url,
...
def parse_question_page(
    ...
    """
    if is_cloudflare_page(html):
        raise SourceError("quora: Cloudflare challenge page — cannot parse")
    ...
    if not title:
        ...
        if title == "Error" or title == "Quora":
            raise SourceError("quora: no question title found")
    ...
    return RawPost(
        id=f"{source}:{qid}" if qid else f"{source}:{abs(hash(question_url))}",
        source=source,
